...
public class fButton extends Button implements Cloneable, Serializable {
    ...
    protected fButton clone() throws CloneNotSupportedException {
        ...
        a.setMaxHeight(this.getMaxHeight());
        a.setMinWidth(this.getWidth());
        a.setHeight(this.getHeight());
    ...
    private void readObject(java.io.ObjectInputStream in)
        ...
        System.out.println("from fButton readObject " + abc);
        setText(abc);
        /*if(abc.equals("যখন তুমি এই বোতামের উপর ক্লিক করো")){
            Controller.spriteClickInitiator();
        }*/
        ...
        if (a.contains("choicebox101")) {
            ...
            for (ch = 'A'; ch <= 'Z'; ++ch) {
                stringChoiceBox.getItems().add(ch + "");
            }
        ...
        else if(a.contains("choicebox102")){
            ChoiceBox<String> colorChoiceBox = new ChoiceBox<>();
            colorChoiceBox.getItems().addAll("লাল","নীল","সবুজ","কালো","সাদা","আকাশি","গোলাপি","হলুদ");
            for(String color : colorChoiceBox.getItems()){
                if(a.contains(color)){
                    colorChoiceBox.setValue(color);
                }
            }
            setGraphic(colorChoiceBox);
        }
        else if (!a.equalsIgnoreCase("nullified"))
            setGraphic(tf);
        if (t.equalsIgnoreCase("r2l"))
        ...
        setMinSize(226, 41);
        setMaxSize(226, 41);
        if(getText().contains("বোতাম")){
            setNodeOrientation(NodeOrientation.LEFT_TO_RIGHT);
            setStyle("-fx-background-color: #FF8000;" + "-fx-text-fill:snow;" +
    ...
    private void writeObject(java.io.ObjectOutputStream out)
        ...
        if (p == null)
            ...
            if(this.getText().contains("যদি"))
                pw.println("choicebox102" + " " + ((ChoiceBox<String>) p).getValue());
            else
                pw.println("choicebox101" + ((ChoiceBox<String>) p).getValue());
        }
        else
            pw.println("nullified");
        pw.flush();
