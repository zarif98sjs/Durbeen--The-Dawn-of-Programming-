...
public class MotionButton extends fButton {
    ...
    public TranslateTransition moveForward(double v) {
    ...
    public RotateTransition rotateIfTrueClockwise(boolean yes, double v) {
        ...
        r1.setOnFinished(event -> {
            System.out.println("Animation finished with v=\n" + v);
            spriteView.find_sprite_position();
        });
        //r1.play();
    ...
    public PathTransition glideToPosition(double time, double x, double y) {
        PathTransition pathTransition = new PathTransition(Duration.seconds(time), new Line(spriteView.getBoundsInLocal().getMinX(), spriteView.getBoundsInLocal().getMinY(), x + 600, y), spriteView);
        ...
        pathTransition.setOnFinished(event -> {
            spriteView.find_sprite_position();
            System.out.println("Path Transition finished");
        });
    ...
    public Transition pointToMousePointer() {
        Point point = MouseInfo.getPointerInfo().getLocation();
        return this.rotateIfTrueClockwise(false, (180.0 / PI) * Math.atan2((point.y - spriteView.getBoundsInLocal().getMaxY()), (point.x - spriteView.getBoundsInLocal().getMaxX())));
    }

    public static TranslateTransition fillerTransition() {
        return new TranslateTransition(Duration.millis(300), new Circle());
    }

    public static TranslateTransition fillerTransition(double seconds) {
        return new TranslateTransition(Duration.seconds(seconds), new Circle());
    }
}
